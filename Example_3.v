Require Import CSSsVerification3.
...
(* 样式_3运行流程案例 *)
Definition Example_3 :=
  (o1_ZD ::=plan_ZD (Atuple4 (192;6;0;1)));;
  (P12 ::=asgn (OId o1_ZD));;
  (o2_ZD ::=plan_ZD (Atuple4 (262;26;1;1)));;
  (P7 ::=asgn (OId o2_ZD));;
  (o1_GRDZT ::=plan_GRDZT (Atuple3 (80;(large_to_nat 7822);(large_to_nat 8422))));;
  (P12 ::=att (OId o1_GRDZT));;
  (IF (BEq ((ANum (triple4_return_third_element ZD1))) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (o1_NMWQZZ ::=plan_NMWQZZ (Atuple3 (80;(large_to_nat 7822);(large_to_nat 8542))));;
  (P12 ::=att (OId o1_NMWQZZ));;
  (o2_GRDZT ::=plan_GRDZT (Atuple3 (80;(large_to_nat 7942);(large_to_nat 8542))));;
  (P7 ::=att (OId o2_GRDZT));;
  (IF (BEq ((ANum (triple4_return_third_element ZD2))) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (o2_NMWQZZ ::=plan_NMWQZZ (Atuple3 (80;(large_to_nat 7942);(large_to_nat 8662))));;
  (P7 ::=att (OId o2_NMWQZZ));;
  (o1_GD ::=plan_GD (Atuple3 (80;(large_to_nat 8542);(large_to_nat 9262))));;
  (P12 ::=att (OId o1_GD));;
  (d1 add 1);;
  (o1_GDDZ ::=plan_GDDZ (Atuple3 (80;(large_to_nat 8542);(large_to_nat 9022))));;
  (P12 ::=att (OId o1_GDDZ));;
  (IF (BEq (ANum D1) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (gdsb_4 sub 1);;
  (o1_JWJC ::=plan_JWJC (Atuple3 (80;(large_to_nat 8542);(large_to_nat 9142))));;
  (P12 ::=att (OId o1_JWJC));;
  (IF (BEq (ANum D1) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (o1_JY ::=plan_JY (Atuple3 (80;(large_to_nat 8542);(large_to_nat 9262))));;
  (P12 ::=att (OId o1_JY));;
  (IF (BEq (ANum D1) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (jyc_12 sub 1);;
  (o2_GD ::=plan_GD (Atuple3 (80;(large_to_nat 8662);(large_to_nat 9382))));;
  (P7 ::=att (OId o2_GD));;
  (d2 add 1);;
  (o2_GDDZ ::=plan_GDDZ (Atuple3 (80;(large_to_nat 8662);(large_to_nat 9142))));;
  (P7 ::=att (OId o2_GDDZ));;
  (IF (BEq (ANum D2) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (gdsb_5 sub 1);;
  (o2_JWJC ::=plan_JWJC (Atuple3 (80;(large_to_nat 8662);(large_to_nat 9262))));;
  (P7 ::=att (OId o2_JWJC));;
  (IF (BEq (ANum D2) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (o2_JY ::=plan_JY (Atuple3 (80;(large_to_nat 8662);(large_to_nat 9382))));;
  (P7 ::=att (OId o2_JY));;
  (IF (BEq (ANum D2) (ANum 1)) THEN SKIP ELSE CAbt FI);;
  (jyc_6 sub 1);;
  (P12 ::exe (ANum 1));;
...
(emp_sO, d1 !sv-> 0; d2 !sv-> 0; gdsb_4 !sv-> 1; gdsb_5 !sv-> 1; jyc_6 !sv-> 1; jyc_12 !sv-> 1; emp_sV, 
 emp_sS, rloc_30 !hr-> 1; rloc_31 !hr-> 1; rloc_32 !hr-> 1; rloc_33 !hr-> 1;  emp_heapR, 
 emp_heapO) =[Example_3]=> Abt.
